...
#            Just keep file names not path
    ...
    def playSong(self):
        if self.songListWidget.currentItem():
            ...
            player.play()
            sc = MyStaticMplCanvas(self.centralwidget, width=2, height=4, dpi=100, index=self.files[self.songListWidget.currentRow()])
            self.tabLayout1.addWidget(sc)
            _translate = QtCore.QCoreApplication.translate
            self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), _translate("MainWindow", self.files[self.songListWidget.currentRow()].split('/')[-1:][0]))
